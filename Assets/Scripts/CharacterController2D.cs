...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
		...
		else if(can2Jump && CrossPlatformInputManager.GetButtonDown("Jump"))
        {
			DoJump();
			can2Jump = false;
        }
    ...
    {
		playerHealth += heal;
		GameManager.gm.AddHeal(heal);
    }

	// public function to apply damage to the player
	public void ApplyDamage (int damage) {
		GameManager.gm.refreshHP(damage);
		...
		if (playerCanMove) {
            ...
            else
            {
				...
				_rigidbody.AddForce(transform.forward*100f,ForceMode2D.Force);
            }
		}
	}
